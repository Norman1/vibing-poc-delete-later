...
# Extract Mark chapter 10
verses = extract_chapter('sources/sblgnt/data/sblgnt/xml/Mark.xml', 'Mark', 10)

# Save to file
with open('mark_10_greek.txt', 'w', encoding='utf-8') as f:
    for verse_num in sorted(verses.keys()):
        f.write(f"Verse {verse_num}: {verses[verse_num]}\n")

print(f"Extracted {len(verses)} verses from Mark chapter 10")
print("Saved to mark_10_greek.txt")

# Print first 3 verses as preview
for i in range(1, min(4, len(verses) + 1)):
    if i in verses:
        preview = verses[i][:100] + "..." if len(verses[i]) > 100 else verses[i]
        print(f"Verse {i}: {preview}")